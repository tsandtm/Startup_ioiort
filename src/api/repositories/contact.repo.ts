...
export class ContactRepo extends RepoBase {
    ...
    public count(option): Promise<number> {
        ...
        return this._query(queryText, [option.TaiKhoan, option.Password])
            .then(result => {
    ...
    public GetUserFacebook(facebook): Promise<any> {
        ...
        return this._query(queryText, [facebook])
            .then(res => {
                if (res.rowCount > 0) {
    ...
    public InsertUserFacebook(user: contact): Promise<any> {
        ...
        return this._query(queryText, [user.Facebook, user.Facebook, user.Email, user.Facebook, user.Token, user.HoTen])
            .then(res => {
                return res;
            }).catch(err => {
                return err;
